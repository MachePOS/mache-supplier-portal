...
const translations = {
  editDeal: { en: 'Edit Deal', fr: 'Modifier l\'offre', ht: 'Modifye òf', es: 'Editar oferta' },
  ...
  save: { en: 'Save Changes', fr: 'Enregistrer', ht: 'Anrejistre', es: 'Guardar cambios' },
  saving: { en: 'Saving...', fr: 'Enregistrement...', ht: 'Ap anrejistre...', es: 'Guardando...' },
  success: { en: 'Deal updated successfully!', fr: 'Offre mise à jour!', ht: 'Òf la mete ajou!', es: '¡Oferta actualizada!' },
  error: { en: 'Error updating deal', fr: 'Erreur lors de la mise à jour', ht: 'Erè pandan mizajou', es: 'Error al actualizar la oferta' },
  loading: { en: 'Loading...', fr: 'Chargement...', ht: 'Ap chaje...', es: 'Cargando...' },
  notFound: { en: 'Deal not found', fr: 'Offre non trouvée', ht: 'Òf pa jwenn', es: 'Oferta no encontrada' },
  delete: { en: 'Delete Deal', fr: 'Supprimer l\'offre', ht: 'Efase òf', es: 'Eliminar oferta' },
  ...
  stats: { en: 'Performance', fr: 'Performance', ht: 'Pèfòmans', es: 'Rendimiento' },
  views: { en: 'Views', fr: 'Vues', ht: 'Vi', es: 'Vistas' },
  conversions: { en: 'Conversions', fr: 'Conversions', ht: 'Konvèsyon', es: 'Conversiones' },
  timesUsed: { en: 'Times Used', fr: 'Utilisations', ht: 'Fwa itilize', es: 'Veces usado' },
  status: { en: 'Status', fr: 'Statut', ht: 'Estati', es: 'Estado' },
  approved: { en: 'Approved', fr: 'Approuvé', ht: 'Apwouve', es: 'Aprobado' },
  pending: { en: 'Pending', fr: 'En attente', ht: 'Ap tann', es: 'Pendiente' },
  rejected: { en: 'Rejected', fr: 'Rejeté', ht: 'Rejte', es: 'Rechazado' },
...
export default function EditDealPage() {
  ...
  const [dealStats, setDealStats] = useState({ views: 0, conversions: 0, times_used: 0, approval_status: 'pending' })
  ...
  const loadDeal = async () => {
    try {
      ...
      // Load deal products if applies_to is specific_products
      if (deal.applies_to === 'specific_products') {
      ...
      setDealStats({
        ...
        conversions: deal.conversions_count || 0,
        times_used: deal.times_used || 0,
        approval_status: deal.approval_status || 'pending',
      })
      ...
      setFormData({
        ...
        free_delivery_threshold: deal.free_delivery_threshold?.toString() || '',
        ...
        is_flash_sale: deal.is_flash_sale || false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // First, delete existing deal products
      await supabase
        .from('marketplace_deal_products')
        .delete()
        .eq('deal_id', dealId)

      // Then, insert new deal products if specific products selected
      if (formData.applies_to === 'specific_products' && selectedProductIds.length > 0) {
        ...
        if (productsError) {
          console.error('Error updating deal products:', productsError)
        }
      }
    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-900">{t('editDeal', translations.editDeal)}</h1>
          <span className={`px-3 py-1 text-sm font-medium rounded-full ${getStatusColor(dealStats.approval_status)}`}>
            {t(dealStats.approval_status, translations[dealStats.approval_status as keyof typeof translations] || { en: dealStats.approval_status })}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
          <p className="text-2xl font-bold text-gray-900">{dealStats.views}</p>
          <p className="text-xs text-gray-500">{t('views', translations.views)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
          <p className="text-2xl font-bold text-gray-900">{dealStats.conversions}</p>
          <p className="text-xs text-gray-500">{t('conversions', translations.conversions)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
          <p className="text-2xl font-bold text-gray-900">{dealStats.times_used}</p>
          <p className="text-xs text-gray-500">{t('timesUsed', translations.timesUsed)}</p>
        </div>
      </div>
      ...
      {success && (
        <div className="mb-6 p-4 bg-green-50 border border-green-200 text-green-700 rounded-lg">
          {t('success', translations.success)}
        </div>
      )}
